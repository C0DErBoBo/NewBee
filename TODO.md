...
- [ ] 登录成功后在前端展示提示并 2-3 秒内自动渐隐，可手动关闭。
...
- [ ] 实现赛事列表页：分页、搜索、排序，支持点击进入赛事详情页编辑配置。
- [ ] 在赛事列表页添加 `新增比赛` 按钮，点击打开赛事配置向导。
- [ ] 构建赛事详情页，按照模块（基础信息、项目、积分、报名规则等）分区编辑并记录变更。
- [ ] 重构赛事配置向导为五步流程（基础信息 → 模板与分组 → 流程与积分 → 成绩与预览设置 → 汇总确认）。
...
## 7. 报名选手流程
- [ ] 实现报名 API，支持选手选择赛事、项目、团队信息与附件上传。
- [ ] 支持报名校验（人数、项目限制、冲突检查），返回清晰错误提示。
- [ ] 构建选手报名管理界面：列表、状态筛选、进度提示、修改撤销能力。
